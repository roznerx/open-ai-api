...
export const CombinedMessages: React.FC<any> = React.memo(
  ({ combinedMessages, fontColor }) => {
    console.log("combinedMessages:", combinedMessages)

    return (
      <>
        {combinedMessages.length > 0 &&
          combinedMessages.map((generatedMessage) => {
            const result = parseText({ text: generatedMessage.message })

            return result.length
              ? result.map((item: any, idx) => {
                  if (item.hasOwnProperty("text")) {
                    return (
                      <div key={idx} className="mr-9 mt-1 flex">
